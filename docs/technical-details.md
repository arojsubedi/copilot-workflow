...
Skill-owned Markdown resources are discovered recursively, including nested `references/` paths, without a fixed resource list. Their source paths are checked for links/redirection before reading; their installed paths receive normal collision and ownership checks. Local edits to active or stale managed resources block mutation.
...
Personal skills and their supplementary resources are installed in GitHub's documented Copilot skill location. The core skill explicitly directs conditional reads of supporting Markdown; discovery alone does not load every procedure into the model context. The clients expose different discovery and inspection controls; use the current client's instruction or skill view rather than assuming that installation proves loading. [GitHub's customization reference](https://docs.github.com/en/copilot/reference/customization-cheat-sheet)
...
Internal `pr-review-` profiles set `infer: false`, the CLI reference's auto-delegation control. GitHub's general agent configuration reference describes that field as retired in favor of other controls; do not assume identical inference behavior across surfaces. Named selection is documented, but parent dispatch with inference disabled must be available in the client in use. If unavailable, keep inference disabled and let the parent cover the question. No mandatory caller is added to planning or implementation: current rubber-duck guidance already covers nontrivial work selectively. [CLI frontmatter](https://docs.github.com/en/copilot/reference/copilot-cli-reference/cli-command-reference#custom-agent-frontmatter-fields), [agent configuration](https://docs.github.com/en/copilot/reference/custom-agents-configuration), [rubber-duck behavior](https://docs.github.com/en/copilot/concepts/agents/copilot-cli/rubber-duck)
