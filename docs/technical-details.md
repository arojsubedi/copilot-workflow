...
Personal CLI agents are installed at `~/.copilot/agents/*.agent.md`; repository agents with matching names take precedence. Verify the selected profile in a fresh client session. Installation at this path does not establish discovery or subagent support in every other surface; inspect each intended client's agent picker. The main PR-review skill covers unavailable delegated perspectives where possible and discloses material gaps. [CLI configuration layout](https://docs.github.com/en/copilot/reference/copilot-cli-reference/cli-config-dir-reference)
