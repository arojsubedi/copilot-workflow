...
MANIFEST_RELATIVE = ".copilot/engineering-workflow/install-manifest.json"
...
def read_routing(text):
    ...
    for line in sections[1].splitlines():
        ...
        if not line.strip():
            continue
        match = re.fullmatch(r"- (Project|Git remote|Jira prefix|Local root): (.+)", line)
        if not match or match[1] in routing:
            raise ValueError("Routing needs unique labeled bullets: Project, Git remote, Jira prefix, optional Local root")
        value = match[2].strip()
    ...
    if not {"Project", "Git remote", "Jira prefix"} <= routing.keys():
        raise ValueError("Routing requires Project, Git remote and Jira prefix (use unset where inapplicable)")
    return routing
...
def project_files(source, report=True):
    ...
    for path in sorted(projects.iterdir()):
        ...
        try:
            ...
            project = routing["Project"].strip()
            if not project or project.casefold() == "unset":
                raise ValueError("Supply a project name")
            ...
            cells = [path.name, project, remote, prefix, local_root]
    ...
    if rows:
        index += ("| Profile | Project | Git remote | Jira prefix | Local root |\n"
                  "| --- | --- | --- | --- | --- |\n" + "\n".join(rows) + "\n")
    else:
...
def load_manifest(home):
    """Return the manifest path and validated ownership entries, or None."""
    manifest = safe_target(home, MANIFEST_RELATIVE)
...
def install(source, home, check=False):
    ...
    project_count = sum(
        1 for relative in files
        if relative.startswith(".copilot/engineering-workflow/projects/")
        and not relative.endswith("/index.md")
    )
    print("Engineering workflow installation/update completed.")
    if project_count:
        print(f"Configured projects: {project_count}.")
    else:
        print("No projects are configured; repository-only work remains available.")
...
def uninstall(home):
    """Remove only unchanged files recorded in this workflow's manifest."""
    home = home.expanduser().resolve()
    manifest, previous = load_manifest(home)
    if previous is None:
        print("Engineering workflow is not installed; no files were removed.")
        print("If you pasted app instructions, clear them manually in Copilot settings.")
        return 0
    ...
    for relative, owned_hash in previous.items():
        ...
        if not target.is_file():
            conflicts.append(str(target) + " is not a file")
        elif digest(target.read_bytes()) != owned_hash:
            conflicts.append(str(target) + " has locally modified installed content")
        else:
            removable.append(target)
    if conflicts:
        raise ValueError(
            "No files removed.\n" + "\n".join(conflicts)
            + "\nBack up wanted changes or restore the installed content, then rerun uninstall."
        )

    for target in removable:
        target.unlink()
    manifest.unlink()
    print(f"Removed {len(removable)} managed files. Unrelated Copilot configuration was preserved.")
    print("If you pasted app instructions, clear them manually in Copilot settings.")
    return 0
