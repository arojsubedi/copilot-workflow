...
Before requesting `rubber-duck`, reconcile any relevant critique already available in current context for the same current conclusion and question, including one initiated by Copilot. Do not mechanically request an equivalent pass. Another independent challenge needs a materially different state/question, changes that make the existing critique stale, or a plausible ability to materially change the conclusion. Keep this in current context without bookkeeping or persistent critique state; prior critique does not belong in neutral independent packets.
