...
Committed `projects/index.example.md` and `projects/project.example.md` define the configuration shape. Ignored local `projects/index.md` and user-named profiles hold actual project facts. Setup installs only the active index and its referenced profiles; installed copies are generated context. This boundary keeps the distribution reusable, permits any number of projects without code changes or a dirty Git working tree, and reduces accidental publication of private metadata. Workflow/template updates leave local facts untouched; new fields are incorporated manually. Configuration transfers between machines separately from Git. No extra directory README, registry, initializer, or template version is needed alongside the examples and onboarding guide.

The index resolves actual Git root and remote identity, including worktrees, or an explicit story target. A Jira prefix cross-checks identity; it never selects another site by trial and error. Remote identity works across machines; an optional exact local-root fallback is machine-specific and unset in the example. The model reads exactly one selected profile. Profiles may be installed UNCONFIGURED for inspection, but remain unusable for live project actions until their facts are verified and status is READY. Placeholder commands/hosts must not be used. A fresh clone installs an explicitly empty UNCONFIGURED index, allowing setup inspection without inventing projects or loading templates. Missing/unconfigured identity does not prevent unrelated local work. Edit local source, rerun setup on each machine, and verify a fresh session; installed profiles are never canonical. Connection names come from the active clients' existing MCP configuration. [Onboarding](docs/setup.md#project-onboarding) covers labeled machine roots/connection names and adding another project.

The installer reads filenames from the existing index's single five-column table. A deliberately narrow plain/backtick filename contract avoids a general Markdown parser or second configuration format. It validates existence, portable relative filenames, and exclusion of templates/linked paths before writes. Unreferenced files stay in source; duplicate identical references install once. Renames, removals, and unreferenced previously installed profiles retain the existing explicit cleanup rule: update/check abort until the maintainer reviews/removes obsolete installed files and their manifest entries. This prevents a rename from activating both copies and preserves installed edits for review.
...
Add a skill only for a coherent recurring procedure with a clear trigger, inputs, result, and mutation boundary. Use a lowercase directory matching frontmatter `name`; descriptions must distinguish nearby tasks. Setup packages files with a lowercase `.md` extension recursively under `skills/` and `writing/`, consistently on both OSes. Under `projects/`, it installs only the active index and its referenced profiles, never the templates. Non-Markdown resources require an intentional installer extension and tests; they are not copied today. Relative skill references and rendered support paths must resolve in the installed tree.
