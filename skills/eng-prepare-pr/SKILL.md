---
name: eng-prepare-pr
description: Prepare or create a pull request, including reviewers and linked Jira context. Use for PR preparation or creation; not for implementing fixes.
---
...
Use current implementation assessment and verification evidence where it covers the pinned comparison, requirements, and relevant conditions. If missing or materially invalidated, use `eng-implementation-review` for a bounded reassessment in review-only mode; tiny clear changes need focused checks. Read [implementation-review](../eng-implementation-review/SKILL.md) if needed. This supplies engineering evidence, not exhaustive PR review or integration approval. Do not refactor during preparation. Unpublished fixes cannot prove the remote head. Carry findings, verification results, pending CI, remaining requested work, and material gaps into the proposal; completing an implementation pass does not establish that the PR's whole scope is complete.

Read `{{WORKFLOW_ROOT}}/writing/style.md` and its PR example if present; apply the style's artifact-requirements rule to live templates and sparse profile overrides. If the title convention uses the Jira summary, preserve that summary. Describe the final behavior and actual verification. Select reviewers from current repository requirements and the profile's configured reviewers where applicable; verify eligibility, exclude the author, and deduplicate. Do not invent substitutes. Default to a non-draft PR only when ready; incomplete work requires an explicitly approved draft.
