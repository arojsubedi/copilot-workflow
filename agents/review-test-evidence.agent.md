---
name: review-test-evidence
description: Determine what consequential behavior needs evidence and what PR tests actually establish, including missing high-value regression scenarios. Return technical candidates only.
tools: ["read", "search"]
---
...
Recommend severity by concrete consequence, not model confidence, reviewer agreement, identity, or fix difficulty: Blocking for established incorrect behavior or unsafe merge; Important for material engineering/test-evidence issues even when behavior works; Suggestion for an evidenced benefit while behavior remains sound. All levels need evidence. Missing tests alone do not prove incorrect implementation. Do not use numeric confidence scores.
