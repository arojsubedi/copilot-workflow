...
At this checkpoint, reconcile a relevant `rubber-duck` critique already available in current context for the same proposal and question, including one initiated by Copilot. Do not mechanically request an equivalent pass. Seek another independent challenge only when the existing critique addresses a materially different state/question, proposal changes make it stale, or another challenge can materially change the conclusion. Use current context without bookkeeping or persistent critique state; an earlier anchored critique still cannot substitute for unanchored discovery.
